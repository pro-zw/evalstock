...
eligible_stock_df = (pd.DataFrame(list(eligible_stocks.values(
    ...
    'roic', 'earnings_yield', 'net_profit_reality')))
    .set_index('stock__stock_name'))

#print(eligible_stock_df[:10])


# 简单指标过滤
eligible_stock_df['市净率'] = stock_quote_df['市净率']
eligible_stock_df = eligible_stock_df[eligible_stock_df['市净率'] <= 3]

eligible_stock_df['市盈率TTM'] = stock_quote_df['市盈率TTM']
eligible_stock_df = eligible_stock_df[eligible_stock_df['市盈率TTM'] <= 20]

# 排序资本回报率
eligible_stock_df.sort_values(by=['roic'], axis=0, ascending=False, inplace=True)
eligible_stock_df['roic_rate'] = range(1, len(eligible_stock_df)+1)
...
# 计算综合排名
eligible_stock_df['overall_rate'] = eligible_stock_df['roic_rate'] + eligible_stock_df['earnings_yield_rate']
eligible_stock_df.sort_values(by=['overall_rate'], axis=0, ascending=True, inplace=True)

#print(len(eligible_stock_df))
#print(eligible_stock_df[:50])
...
f, ax = plt.subplots()
ax.scatter(eligible_stock_df['roic'], eligible_stock_df['earnings_yield'])
ax.set_title('神奇公式分布图')
ax.set_xlabel('资本回报率')
ax.set_ylabel('收益率')
for i, stock_name in enumerate(eligible_stock_df['stock__stock_name']):
    ax.annotate(stock_name, (eligible_stock_df['roic'].iloc[i], eligible_stock_df['earnings_yield'].iloc[i]))
plt.show()